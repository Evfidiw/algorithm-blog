...
- 思想：每次检查相邻两个元素，如果前面的元素与后面的元素满足给定的排序条件，就将相邻两个元素交换。当没有相邻的元素需要交换时，排序就完成了。
    由于在算法的执行过程中，较小的元素像是气泡般"冒泡"到数列的顶端（相应大的泡泡则会“沉”下去，也就是**每一次外层循环会确定尾端的数**）故叫做冒泡排序。
...
- 思想：
    ...
    - 其实是冒泡排序的升级版：冒泡排序可以理解为一个元素为基准与下一个元素比较；而快排则是选定任意一个基准，划分成比基准大或者小的两组，每组再分别重复进行，直到无法继续划分。
...
- 时间复杂度：
    ...
    - 最好情况: T(n) = 2T(n/2) + O(n)
    - 平均情况: **任何一种**常数比例的划分都会产生深度为O(logn) 的递归树，每一层的时间代价都是O(n)
