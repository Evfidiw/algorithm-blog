...
4. 无脑计算KMP相关的 三个数组 和 其作用：
    ...
    1. 部分匹配表（PM）：
        ...
        - 算法流程：（[可视化](https://www.bilibili.com/video/BV1AY4y157yL/?spm_id_from=333.337.search-card.all.click&vd_source=c15853b3823e1d0ffa37003db0d4dd3f]，需要注意的是视频中的"数组"实际上是PM表)）
            ...
            1. 得到PM表
            2. 匹配搜索：当子串 i 处不匹配时，记PM值 i-1 处的值为 X ，子串跳过前X个位置，从**子串的第X+1个位置**和**主串的不匹配处**继续匹配；

    2. next数组：

        - 含义：当子串 i 处不匹配时，记next数组 i 处的值为X，从子串的后 X 个字符继续匹配（下标从1开始就从 X 个匹配，下标从0开始从 X+1 个匹配）

        - **注意**，教材默认下标为1，next的值为0（第一个）就是连第一个都不匹配继续匹配下一串吧（也就是主串和子串的位置和指针都移到下一个位置），next的值为 X 则是要从将子串指针指向第X个重新匹配。（不太理解？用next数组模拟前面的可视化图的变化）

        - 也就是说，利用PM表是子串指针**跳过**X个字符，利用next数组是子串指针**指向**X个字符（仔细体会两者的关系…）
