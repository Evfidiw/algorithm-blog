...
3. KMP算法
    ...
    - 利用 nextval 数组改进：
        ...
        1. 把next数组换成nextval数组
            - 如果 `P[i] != P[next[i]]`，则 `nextval[i] = next[i]`
            - 如果 `P[i] == P[next[i]]`，则 `nextval[i] = nextval[next[i]]`
        2. 匹配过程
            - 如果模式字符串中的字符匹配（`pattern[i] == pattern[j]`），则`nextval[i] = j + 1`，同时`i`和`j`都递增。
            - 如果不匹配且`j`不为0，则将`j`更新为`nextval[j]`
            - 如果不匹配且`j`为0，则`nextval[i] = 0`，并且`i`递增
        ...
        >
        > **更要注意的是**，跳过X个字符，利用next数组是不匹配处的前一个字符作为索引，利用nextval是不匹配处作为索引
